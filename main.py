...
async def build_news_from_events(
    ...
    for group_index, (tag, group_events) in enumerate(groups.items(), start=1):
        ...
        # IMPORTANT: do not ask the final writer to read the raw RP posts and
        # retell them. First compress the whole tagged storyline into a small
        # set of verified facts. This prevents long RP scenes from consuming
        # the completion budget and makes the final text concise.
        digest_result = await groq(
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Ты информационный редактор. Перед тобой несколько постов "
                        "ОДНОЙ продолжительной сюжетной линии. Твоя задача — не писать "
                        "новость, а сделать КРАТКУЮ ФАКТИЧЕСКУЮ ВЫЖИМКУ для другого "
                        "редактора.\n\n"
                        "Оставляй только действительно важное для сюжета: кто, что "
                        "сделал, что произошло, где произошло, важные последствия, "
                        "решения, конфликты и изменения ситуации.\n"
                        "Убирай художественные описания, атмосферу, внутренние "
                        "размышления, повторения, длинные диалоги, несущественные "
                        "детали и красивости.\n"
                        "НЕ ВЫДУМЫВАЙ фактов и не делай выводов, которых нет в постах. "
                        "Разные сцены всё равно являются одной сюжетной линией. "
                        "Сохраняй хронологию.\n"
                        "Верни максимум 8–12 коротких фактических пунктов. "
                        "Имена персонажей, места и ключевые действия сохраняй."
                    ),
                },
                {
                    "role": "user",
                    "content": (
                        ...
                        "Сделай компактную выжимку из материалов ниже.\n\n"
                        f"{source}"
                    ),
                },
            ],
            model=GROQ_MODEL,
            temperature=0.15,
            purpose=f"news_story_digest_{group_index}",
            max_completion_tokens=650,
        )

        digest = ""
        try:
            digest = (digest_result["choices"][0]["message"]["content"] or "").strip()
        except (KeyError, IndexError, TypeError, AttributeError):
            logger.exception(
                "NEWS DIGEST INVALID group=%s title=%s",
                group_index, tag,
            )

        if not digest:
            # Keep a small fallback rather than feeding the entire RP archive
            # into the writer.
            digest = source[:7000]

        logger.info(
            "NEWS DIGEST READY group=%s title=%s source_chars=%s digest_chars=%s",
            group_index, tag, len(source), len(digest),
        )

        prompt = (
            f"{mode}.\n\n"
            f"СЮЖЕТНАЯ ЛИНИЯ: #{tag}\n\n"
            "Ты получаешь НЕ исходные посты, а уже проверенную выжимку. "
            "На её основе напиши РОВНО ОДИН короткий новостной блок.\n\n"
            "Главная цель — передать СМЫСЛ И РАЗВИТИЕ СОБЫТИЯ, а не пересказать "
            "каждый пост. Выбирай только самые значимые изменения ситуации. "
            "Не нужно упоминать всех участников и все действия.\n"
            "Не повторяй одну и ту же мысль разными словами. Не перечисляй "
            "события через длинную цепочку. Не добавляй художественные детали. "
            "Если есть несколько сцен, свяжи их как развитие одной сюжетной линии. "
            "Разные персонажи, игроки и локации не делают её разными событиями. "
            "Ничего не выдумывай.\n\n"
            "Формат: 1 короткий абзац, при необходимости максимум 2. "
            "Ориентир — 500–800 знаков. ОБЯЗАТЕЛЬНО закончи мысль полным "
            "предложением.\n\n"
            "ВЫЖИМКА ФАКТОВ:\n"
            f"{digest}"
        )
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Пиши коротко и по существу: только главное развитие события. "
                        "Не пересказывай исходные посты. Не выдумывай факты."
                    ),
                },
            ...
            repair = await groq(
                messages=[
                    ...
                    },
                    {
                        ...
                        "content": (
                            f"Сюжет: #{tag}\n\n"
                            "Напиши короткую сводку в 1–2 абзацах, только главное, и обязательно "
